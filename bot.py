...
def update_league_positions():
    """Обновляет позиции всех клубов на основе их текущего рейтинга"""
    for (league, country, tier), clubs in LEAGUES.items():
        # Сортируем клубы по рейтингу (чем выше рейтинг, тем лучше позиция)
...
def simulate_club_performance(club, year):
    """Симулирует выступление клуба и возвращает список выигранных трофеев."""
    trophies = []
    base_rating = club.get_rating(year)
    
    # Шанс выиграть Лигу чемпионов (только для клубов с высоким рейтингом)
    if random.random() < 0.05 * (base_rating / 50):
        trophies.append(f"🏆 Лига чемпионов {year}")
        club.trophies.append(f"Лига чемпионов {year}")
    
    # Шанс выиграть национальный кубок
...
def check_promotion_relegation(club):
    """
    Проверяет, должен ли клуб повыситься или вылететь.
    Возвращает список сообщений о произошедших изменениях.
    """
    messages = []
    country = club.country
    # Если клуб не в 1 или 2 дивизионе, ничего не делаем
    if club.league_tier not in (1, 2):
        return messages

    # Находим все клубы в той же стране и лиге
    same_league_clubs = [c for c in ALL_CLUBS if c.league == club.league and c.country == country]
    league_size = len(same_league_clubs)

    # Проверяем вылет из первого дивизиона (последнее место)
    if club.league_tier == 1 and club.league_position == league_size:
        # Ищем клуб во втором дивизионе с наилучшей позицией
        ...
        if second_league_clubs:
            ...
            if best_second.league_position == 1:
                ...
                # Меняем названия лиг согласно словарю (с защитой от отсутствия ключа)
    ...
    # Проверяем повышение из второго дивизиона (первое место)
    elif club.league_tier == 2 and club.league_position == 1:
        # Ищем клуб в первом дивизионе с наихудшей позицией
...
def simulate_year(player):
    ...
    # Симуляция выступления клуба (только трофеи, позиция будет обновлена позже)
    ...
    # Проверка повышения/вылета (до увеличения года)
    ...
    # Увеличиваем возраст и год
    player.age += 1
    player.current_year += 1
    
    # Генерация трансферных предложений на следующий сезон
...
async def get_country(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Ищем клуб из второй лиги (рейтинг 40-55) в той же стране
